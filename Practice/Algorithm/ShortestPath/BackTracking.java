/*
 * 최단 거리 구하기 알고리즘
 * Backtracking: 가능한 모든 경우를 살펴보고 원하는 답(최소/최대값)을 찾는 방식
 * --> 여기서는 '트리'상태의 상태공간을 DFS방식으로 탐색함
 */
...
class ShortestPath {
	...
	public void findNextNode(int currentNode, int destination, double distance) { 
		...
		for (int i = 0; i < matrix.length; i++) {
			if (!mark[i] && matrix[currentNode][i] > 0 && (distance + matrix[currentNode][i] < minDist)) { // 가지치기: 가망 있는 경우만 따지겠다!
				findNextNode(i, destination, distance + matrix[currentNode][i]);
				
				for (int j = 0; j < mark.length; j++) {
					mark[j] = backup[j]; // 이전 나뭇가지(재귀 호출)과 독립적인 또다른 나뭇가지!
				}
			}
		}
	}
